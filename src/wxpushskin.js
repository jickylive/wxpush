export default {
  async fetch(request, _env, _ctx) {
    ...
    const title = escapeHtml(url.searchParams.get('title') || '消息推送')
    const message = escapeHtml(url.searchParams.get('message') || '无告警信息')
    const date = escapeHtml(url.searchParams.get('date') || '无时间信息')
    const html = `
  <!DOCTYPE html>
  <html lang="zh-CN">
  <head>
